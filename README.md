...
**Что нужно на чистой машине:**

- **Foundry** — `anvil`, `forge` и `cast`, ставятся вместе, шаги ниже;
- **python3** — им поднимается статика витрины. Если его нет, `run.sh`
  возьмет `php -S` или `ruby -run -e httpd`, но python3 — основной путь;
- **репозиторий, склонированный с подмодулями**. Скачивание архивом
  (Download ZIP) подмодули не приносит: `lib/forge-std` и
  `lib/openzeppelin-contracts` останутся пустыми, поэтому лучше клонировать.
  Если архив уже скачан, `run.sh` доставит зависимости через `forge install` —
  для этого нужна сеть.

**1. Foundry.** Без него `run.sh` не работает — из Foundry берутся `anvil`,
`forge` и `cast`:
...
Установщик правит профиль оболочки, поэтому **перезапустите терминал**
(или откройте новую вкладку), затем поставьте сами инструменты и проверьте
версию:
...
Если `forge --version` печатает версию — Foundry на месте.

**2. Репозиторий.** Клонировать **обязательно с подмодулями** — `forge-std` и
...
**3. Запуск одной командой.** Скрипт `./run.sh` делает ровно то, что расписано
ниже на три терминала: проверяет инструменты (и, если Foundry не найден,
печатает те же шаги установки), доставляет зависимости контрактов, если папки
подмодулей пусты, собирает контракты `forge build` и останавливается с внятной
ошибкой, если сборка не прошла, гасит процессы предыдущего запуска на портах
8545 и 8000, поднимает узел, разворачивает контракты, поднимает статику
и открывает витрину. Сам ничего не устанавливает и профиль оболочки не правит.
Логи узла, сборки и сервера пишутся в `.logs/`, в терминал идут только понятные
строки. Ctrl+C гасит оба фоновых процесса и убирает за собой.
